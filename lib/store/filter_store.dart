...
abstract class FilterStoreBase with Store {
  ...
  @observable
  List<int> shopIds = ObservableList();
  ...
  @action
  Future<void> addToShopList({required int serId}) async {
    shopIds.add(serId);
    updateFilterFlag();
  }

  @action
  Future<void> removeFromShopList({required int serId}) async {
    shopIds.removeWhere((element) => element == serId);
  }
}
