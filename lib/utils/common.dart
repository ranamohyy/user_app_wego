...
void showNewUpdateDialog(BuildContext context, {required int currentAppVersionCode}) async {
  showInDialog(
    ...
    builder: (_) {
      return PopScope(
        canPop: currentAppVersionCode >= getIntAsync(USER_APP_MINIMUM_VERSION).toInt(),
        child: NewUpdateDialog(canClose: getBoolAsync(AUTO_UPDATE, defaultValue: true) ? true : currentAppVersionCode >= getIntAsync(USER_APP_MINIMUM_VERSION).toInt()),
      );
    },
  );
}
...
String formatShopTime(String? dateTimeStr) {
  ...
  try {
    ...
    if (dateTimeStr.contains('T')) {
      ...
      if (parts.length >= 2) {
        ...
        dt = DateTime(2023, 1, 1, hour, minute);
      } else {
        return '';
      }
    } else if (dateTimeStr.contains('.') || dateTimeStr.contains('-')) {
      String normalized = dateTimeStr.replaceAll('.', ':').replaceAll('-', ':');
    ...
    String timeFormat = getStringAsync(TIME_FORMAT);
    String format = timeFormat.isNotEmpty ? timeFormat : 'hh:mm a';
